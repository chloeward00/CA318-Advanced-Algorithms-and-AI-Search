      ...
      if next == goal:
         ...
         for child in next.get_children():
            if child not in visited and child not in todo:
               child.score = child.depth + child.out_place(goal)  # score = g(n) + h(n)
               todo.append(child)
...
def main(args):
   ...
   num_searches, end = a_star_search(start, goal) # Does the BFS while having start and goal initialised as a star search
   if end == None:
